...
// update lips by ludy 
if (this.lip_index < this.lips.length) {
    this.live2DModel.setParamFloat('PARAM_MOUTH_OPEN_Y', this.lips[this.lip_index]);
    this.lip_index++;
}
...
if (this.modelSetting.getMotionSound(name, no) == null) {
    ...
    xhr.onreadystatechange = function() {
        ...
        if (status === 0 || status === 200) {
            thisRef.lips = xhr.response.split(' ')
            for (var i = 0; i < thisRef.lips.length; i++)
                thisRef.lips[i] = parseFloat(thisRef.lips[i]);
        } else {
            throw new Error('Could not load ' + lip_src);
        }
    };
